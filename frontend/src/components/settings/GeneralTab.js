...
const GeneralTab = () => {
  ...
  // Personal override (issue #91): saved server-side against this user's
  // account, not just this browser, so it follows them to another device.
  // Explicit "no override" isn't offered here — picking a language always
  // sets one; clearing back to "inherit the admin's default" isn't a common
  // enough need to add a control for yet.
  const handleLanguageChange = (code) => {
    changeLanguage(code);
    authAPI.updateProfile({ language: code }).catch((err) => console.error('Error saving language preference:', err));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-lg font-semibold mb-4">{isAdmin ? t('settings.general.yourPreferences') : t('settings.general.appConfiguration')}</h3>
        <div>
